...
def resolve_params() -> dict:
    ...
    resolved = resolve_runtime_params(context)
    log_event(
        service="airflow",
        module="pipeline_competition",
        step="resolve_params",
        status="success",
        context=context,
        dataset="pipeline",
        message=f"[pipeline_competition] Params resolvidos: {resolved}",
    )
    return resolved
...
def log_stage(stage: str, action: str):
    ...
    log_event(
        ...
        context=context,
        dataset="pipeline",
        message=f"[pipeline_competition] {action} | stage={stage} | run_id={context.get('run_id')}",
    )


with DAG(
    dag_id="pipeline_competition",
    ...
    tags=["pipeline", "orchestrator", "competition"],
) as dag:
    resolve_runtime = PythonOperator(
    ...
    start_pipeline = PythonOperator(
        ...
        op_kwargs={"stage": "pipeline_competition", "action": "START"},
        execution_timeout=timedelta(minutes=5),
    )
    end_pipeline = PythonOperator(
        task_id="end_pipeline",
        python_callable=log_stage,
        op_kwargs={"stage": "pipeline_competition", "action": "END"},
        execution_timeout=timedelta(minutes=5),
    )

    resolve_runtime >> start_pipeline
